#!/usr/bin/env python3
"""geo-geochem 地球化学异常处理系统 - Flask 应用。

独立服务：上传研究区 KML/KMZ + 选矿种（+ 可选 ICP-MS/XRF 点位 CSV）→
点位插值/区域阈值先验 → C-A 背景/异常分离 + 多元素组合异常 → 化探异常成果。
"""
...
from core.geochem_engine import GeochemEngine
...
def start_analysis():
    ...
    try:
        ...
        # 可选：化探点位 CSV 上传
        geochem_path = None
        gfile = request.files.get('geochem')
        if gfile and gfile.filename:
            gext = gfile.filename.rsplit('.', 1)[-1].lower()
            if gext in ('csv', 'txt'):
                geochem_path = os.path.join(
                    Config.TEMP_FOLDER,
                    f"geochem_{int(time.time())}_{re.sub(r'[^A-Za-z0-9._-]+','_',gfile.filename)}")
                gfile.save(geochem_path)
        ...
        task_id = f"geochem_{ts}_{task_counter:03d}"
        task_counter += 1
        run_id = ts + '_' + f"{task_counter:03d}"
        output_dir = os.path.join(Config.RESULTS_FOLDER, safe_aoi, 'geochem', run_id)

        params = {'geochem_path': geochem_path}
        ...
        def run_task():
            ...
            try:
                ...
                res = GeochemEngine.run(aoi_name, mineral, bbox, output_dir,
                ...
                import traceback
                logger.error(f"化探处理失败: {traceback.format_exc()}")
...
def chain_model3d(task_id):
    """一键串联：把本任务的区域+矿种提交给 geo-model3d(8086)。"""
    task = _resolve_task(task_id)
