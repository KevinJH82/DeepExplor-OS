...
def build_deformation_evidence(aoi_dir: Path) -> Optional[Dict]:
    ...
    # 目标范围:优先裁到 ROI(AOI bbox) —— S1 burst 远大于小 AOI,不裁会输出整景。
    # 取并集∩ROI;小 AOI 完全落在 burst 内时即精确等于 ROI。
    full_extent = list(union)
    roi_extent = list(union)
    roi = aoi_meta.get("aoi_bbox")
    if isinstance(roi, (list, tuple)) and len(roi) == 4:
        rx0, rx1 = sorted((roi[0], roi[2]))
        ry0, ry1 = sorted((roi[1], roi[3]))
        cminx, cminy = max(union[0], rx0), max(union[1], ry0)
        cmaxx, cmaxy = min(union[2], rx1), min(union[3], ry1)
        if cmaxx > cminx and cmaxy > cminy:
            roi_extent = [cminx, cminy, cmaxx, cmaxy]

    # ── 2) 各 burst |velocity| 重投影到目标网格，逐像元取 max ──
    def _mosaic_for(ext):
        ex0, ey0, ex1, ey1 = ext
        w = max(1, int(math.ceil((ex1 - ex0) / res_deg)))
        h = max(1, int(math.ceil((ey1 - ey0) / res_deg)))
        dt = from_origin(ex0, ey1, res_deg, res_deg)
        mo = np.full((h, w), np.nan, dtype=np.float32)
        used = 0
        for tif in vel_tifs:
            try:
                with rasterio.open(tif) as src:
                ...
                with rasterio.open(tif) as src:
                    reproject(source=arr, destination=buf,
        ...
        return mo, dt, w, h, used

    # 优先 ROI 裁剪;若该区相干像元稀疏导致裁剪后全空(如热带低相干区),回退全范围以保证出图。
    mosaic, dst_transform, width, height, n_used = _mosaic_for(roi_extent)
    if n_used and not np.isfinite(mosaic).any() and roi_extent != full_extent:
        print("    [回退] ROI 裁剪后无有效形变像元(相干性低),改用全范围出图")
        mosaic, dst_transform, width, height, n_used = _mosaic_for(full_extent)
    ...
    # 决策轨迹血缘三键（容错，不影响产物）：上游 AOI meta 含 trace_id 则继承，否则自生成
    try:
        import sys as _sys
        ...
        stamp_metadata(meta, explicit_trace_id=aoi_meta.get("trace_id"), tenant_id=aoi_meta.get("tenant_id"))
    except Exception:
        pass
