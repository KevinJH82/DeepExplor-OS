...
def api_preflight_fix():
    """
    一键修复前置检查中可自动修复的问题。
    Body JSON: { "actions": ["pip"] }
    目前支持:
      - "pip": 运行 venv/bin/pip install -r requirements.txt 安装缺失依赖
    """
        ...
        if action == "pip":
            if not pip_path.exists():
                results.append({
                    "action": "pip",
                    "success": False,
                    "output": f"venv pip 不存在: {pip_path}",
                })
                continue
            try:
                proc = subprocess.run(
                ...
                results.append({
                    ...
                    "output": output[-2000:],  # 截取最后 2000 字符避免过长
                })
            except subprocess.TimeoutExpired:
                results.append({
                    "action": "pip",
                    "success": False,
                    "output": "pip install 超时(120s)",
                })
