...
# 检查结果数组
declare -a NAMES STATUSES DETAILS FIXES

record() {
  NAMES+=("$1")
  STATUSES+=("$2")
  DETAILS+=("$3")
  FIXES+=("${4:-}")   # fix 类型: "pip" / "manual" / ""(不可自动修复)
}
...
if [[ -n "$port_skip" ]]; then
  ...
  if [[ "$proc_cwd" == "$PROJECT_DIR"* ]] || [[ "$proc_args" == *"$PROJECT_DIR"* ]]; then
    ...
    record "端口 8084" "fail" "已被外部进程占用: $port_proc (PID: $port_pid)${proc_cwd:+, cwd=$proc_cwd}" "manual"
  fi
fi
...
$PYTHON_BIN -c "import sys; sys.exit(0)" 2>/dev/null || {
...
check_pkg() {
  ...
  if $PYTHON_BIN -c "import $pkg" 2>/dev/null; then
    local ver=$($PYTHON_BIN -c "import $pkg; print(getattr($pkg, '__version__', '?'))" 2>/dev/null)
    record "Python: $pkg" "pass" "$ver"
  else
    record "Python: $pkg" "fail" "未安装(运行 $PROJECT_DIR/venv/bin/pip install $pkg)" "pip"
  fi
}
check_pkg "hyp3_sdk"
...
check_pkg "flask"
check_pkg "rasterio"
check_pkg "yaml"  # PyYAML (Python module name: yaml, pip name: PyYAML)
check_pkg "lxml"
check_pkg "shapely"
...
if [[ $JSON_MODE -eq 1 ]]; then
  ...
  for i in "${!NAMES[@]}"; do
    ...
    fix="${FIXES[$i]}"
    if [[ -n "$fix" ]]; then
      echo -n "{\"name\":\"$name\",\"status\":\"$status\",\"detail\":\"$detail\",\"fix\":\"$fix\"}"
    else
