...
def parse_kml(kml_path: str, point_buffer_deg: float = DEFAULT_POINT_BUFFER_DEG):
    ...
    # Excel 文件：委托给 excel_parser
    if suffix in (".xlsx", ".xls"):
        from .excel_parser import parse_excel, ExcelParseError
        try:
            return parse_excel(str(kml_path), point_buffer_deg)
        except ExcelParseError as e:
            raise KMLParseError(str(e))

    if suffix not in (".kml", ".ovkml", ".kmz", ".ovkmz"):
        raise KMLParseError(f"不支持的文件格式: {kml_path.suffix}（仅支持 .kml / .ovkml / .kmz / .ovkmz / .xlsx / .xls）")
...
def parse_kml_folder(folder_path: str, point_buffer_deg: float = DEFAULT_POINT_BUFFER_DEG) -> List[Tuple]:
    """批量解析文件夹中所有KML文件，返回 [(geometry, bbox, name), ...]"""
    _check_dependencies()

    folder = Path(folder_path)
    if not folder.is_dir():
        raise KMLParseError(f"路径不是文件夹: {folder_path}")

    kml_files = sorted(
        list(folder.glob("*.kml")) + list(folder.glob("*.KML")) +
        list(folder.glob("*.ovkml")) + list(folder.glob("*.OVKML")) +
        list(folder.glob("*.kmz")) + list(folder.glob("*.KMZ")) +
        list(folder.glob("*.ovkmz")) + list(folder.glob("*.OVKMZ")) +
        list(folder.glob("*.xlsx")) + list(folder.glob("*.XLSX")) +
        list(folder.glob("*.xls")) + list(folder.glob("*.XLS"))
    )
    if not kml_files:
        raise KMLParseError(f"文件夹中未找到坐标文件（.kml / .ovkml / .kmz / .ovkmz / .xlsx / .xls）: {folder_path}")

    results = []
    for kml_file in kml_files:
        print(f"  解析 KML: {kml_file.name}")
        try:
            results.append(parse_kml(str(kml_file), point_buffer_deg))
        except KMLParseError as e:
            print(f"  [警告] 跳过 {kml_file.name}: {e}")

    return results


def bbox_to_geojson(bbox: Tuple) -> dict:
    """BBox → GeoJSON Polygon（STAC API 查询用）"""
